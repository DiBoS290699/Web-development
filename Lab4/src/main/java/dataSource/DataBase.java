...
public class DataBase {
    ...
    public void insertService(Service service, int ID) throws SQLException {
        Connection connect = getConnection();
        PreparedStatement state = connect.prepareStatement("insert into service(type, price, count, cost) values(?,?,?,?)");
        PreparedStatement state1 = connect.prepareStatement("insert into visit(id, type_service) values(?,?)");
        state.setString(1, service.getType());
        state.setDouble(2, service.getPrice());
        state.setInt(3, service.getCount());
        state.setDouble(4, service.getCost());
        state.execute();
        state1.setInt(1, ID);
